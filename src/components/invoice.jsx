...
import { toPersianDigits, formatNumber, parseNumber, numberToPersianWords, getCurrentPersianDate, getCustomerCode, getAutoInvoiceNumber } from "../utils/invoiceHelpers";
...
function Invoice({
  ...
  const [invoiceType, setInvoiceType] = useState(invoiceToEdit ? invoiceToEdit.type || "پیش فاکتور" : "پیش فاکتور");
  const [invoiceDate, setInvoiceDate] = useState(invoiceToEdit ? invoiceToEdit.date || getCurrentPersianDate() : getCurrentPersianDate());

  // اطلاعات فروشنده (بارگیری از تنظیمات)
  const [sellerName] = useState(() => localStorage.getItem("companyName") || "نام شرکت خود را وارد کنید");
  const [sellerAddress] = useState(() => localStorage.getItem("companyAddress") || "آدرس شرکت خود را وارد کنید");
  const [sellerPhone] = useState(() => localStorage.getItem("companyPhone") || "شماره تماس شرکت");
  const [sellerEconomicCode] = useState(() => localStorage.getItem("companyEconomicCode") || "");
  const [companyLogo] = useState(() => localStorage.getItem("companyLogo") || "");
  const handleBack = () => {
  ...
  const [selectedCustomerId, setSelectedCustomerId] = useState(invoiceToEdit && invoiceToEdit.customerId ? invoiceToEdit.customerId : initialCustomer ? initialCustomer.id : "");
  const [buyerName, setBuyerName] = useState(initialCustomer ? initialCustomer.name : "");
  const [buyerEconomicCode, setBuyerEconomicCode] = useState(initialCustomer && initialCustomer.economicCode ? initialCustomer.economicCode : "");
  const [buyerAddress, setBuyerAddress] = useState(initialCustomer && initialCustomer.address ? initialCustomer.address : "");
  const [buyerMobile, setBuyerMobile] = useState(initialCustomer && initialCustomer.phone ? initialCustomer.phone : "");
  const [saveAsNewCustomer, setSaveAsNewCustomer] = useState(false);
  const [isDatePickerOpen, setIsDatePickerOpen] = useState(false);

  // محاسبه اولیه شماره فاکتور
  const initialCustObj = customers.find(c => String(c.id) === String(selectedCustomerId));
  ...
  const [invoiceNumber, setInvoiceNumber] = useState(() => {
    ...
    return getAutoInvoiceNumber(initialCustCode, invoiceType, invoices, selectedCustomerId);
  });

  // تابع به‌روزرسانی شماره فاکتور خودکار
  const updateAutoInvoiceNumber = (custUserId, typeVal, invList = invoices) => {
    const currentCust = customers.find(c => String(c.id) === String(custUserId));
    const custCode = getCustomerCode(currentCust, customers);
    const autoNum = getAutoInvoiceNumber(custCode, typeVal, invList, custUserId);
    setInvoiceNumber(autoNum);
  };
  const PROFORMA_DEFAULT_NOTE = "به دلیل نوسانات بازار این پیش فاکتور تا زمان دریافت اسناد مالی قابل تغییر قیمت می‌باشد و فروشنده تضمینی در قبال مبلغ ندارد.";
  const INVOICE_DEFAULT_NOTE = "اقلام فاکتور تا زمان تسویه حساب کامل نزد خریدار به صورت امانت می باشد";
  const [noteText, setNoteText] = useState(() => {
    if (invoiceToEdit && invoiceToEdit.note !== undefined) return invoiceToEdit.note;
    const initialType = invoiceToEdit ? (invoiceToEdit.type || "پیش فاکتور") : "پیش فاکتور";
    return (initialType === "فاکتور فروش" || initialType === "فاکتور") ? INVOICE_DEFAULT_NOTE : PROFORMA_DEFAULT_NOTE;
  });
  ...
  const [rows, setRows] = useState(() => {
    if (invoiceToEdit && Array.isArray(invoiceToEdit.items) && invoiceToEdit.items.length > 0) {
  ...
  const showAlert = (msg) => {
    return new Promise((resolve) => {
      setCustomAlert({
        ...
        type: "alert",
        onConfirm: () => {
          setCustomAlert({ show: false, message: "", type: "alert", onConfirm: null, onCancel: null });
          resolve(true);
        },
        onCancel: () => {
          setCustomAlert({ show: false, message: "", type: "alert", onConfirm: null, onCancel: null });
  ...
  const grandTotal = rows.reduce((acc, row) => {
    const q = row.quantity === "" || row.quantity === undefined ? 1 : parseNumber(row.quantity) || 0;
    const p = row.unitPrice === "" || row.unitPrice === undefined ? 0 : parseNumber(row.unitPrice) || 0;
    return acc + q * p;
  }, 0);
  ...
  // --- تابع ثبت نهایی فاکتور و ذخیره مشتری جدید در صورت نیاز ---
    ...
    if (!targetCustomerId && buyerName.trim()) {
      ...
      if (await showConfirm(confirmMsg)) {
        const existingCust = (customers || []).find(c => c.name && c.name.trim().toLowerCase() === buyerName.trim().toLowerCase());
        if (existingCust) {
          await showAlert("مشتری با این نام از قبل وجود دارد. لطفاً از لیست انتخاب کنید یا نام دیگری وارد نمایید.");
          return;
        }
        ...
        if (savedOk) {
          ...
          const newlyAdded = updatedCustList.find(c => c.name && c.name.trim().toLowerCase() === buyerName.trim().toLowerCase());
          if (newlyAdded) {
    ...
    if (!targetCustomerId) {
      await showAlert("یک حساب از لیست انتخاب کنید یا مشخصات مشتری جدید را تکمیل کنید.");
      return;
    }
    ...
    try {
      const invoiceData = {
        ...
        items: JSON.stringify(rows.map(r => {
          return {
            ...
            quantity: r.quantity === "" || r.quantity === undefined ? 1 : parseNumber(r.quantity),
            unitPrice: r.unitPrice === "" || r.unitPrice === undefined ? 0 : parseNumber(r.unitPrice)
          };
        })),
      ...
      const typeLabel = invoiceType === "فاکتور فروش" || invoiceType === "فاکتور" ? "فاکتور" : "پیش فاکتور";
      if (isUpdate) {
      ...
      {step === "form" && <>
        <div className="invoice-form-container">
          ...
          <div className="invoice-form-card">
            ...
            <div className="invoice-form-grid-2">
              <div className="invoice-field-group">
                ...
                <CustomSelect value={invoiceType} onChange={newType => {
              setInvoiceType(newType);
              updateAutoInvoiceNumber(selectedCustomerId, newType);
              if (noteText === PROFORMA_DEFAULT_NOTE || noteText === INVOICE_DEFAULT_NOTE || !noteText.trim()) {
                setNoteText((newType === "فاکتور فروش" || newType === "فاکتور") ? INVOICE_DEFAULT_NOTE : PROFORMA_DEFAULT_NOTE);
              }
            }} options={[{
              value: "پیش فاکتور",
              label: "پیش فاکتور"
              ...
              <div className="invoice-form-grid-2-inner">
                <div className="invoice-field-group">
                  ...
                  <input type="text" dir="ltr" style={{ textAlign: "right" }} className="invoice-input-text" value={invoiceNumber} onChange={e => setInvoiceNumber(toPersianDigits(e.target.value))} />
                </div>
                <div className="invoice-field-group">
                  <label className="invoice-field-label">تاریخ فاکتور</label>
                  <div className="invoice-input-relative">
                    <input type="text" className="invoice-input-text date-input-field" value={invoiceDate} onChange={e => setInvoiceDate(toPersianDigits(e.target.value))} />
                    <button type="button" className="invoice-calendar-trigger-btn" onClick={() => setIsDatePickerOpen(true)} title="انتخاب تاریخ از تقویم">
          ...
          <div className="invoice-form-card">
            ...
            <div className="invoice-field-group">
              <label className="invoice-field-label">انتخاب مشتری از لیست</label>
              <CustomSelect value={selectedCustomerId || ""} onChange={custId => handleCustomerSelect(custId)} options={[{
            value: "",
            label: "-- مشتری جدید --"
          }, ...customers.map(c => ({
            value: c.id,
            ...
            <div className="invoice-form-grid-2">
              <div className="invoice-field-group">
                <label className="invoice-field-label">نام خریدار / شرکت *</label>
                <input type="text" className="invoice-input-text" value={buyerName} readOnly={!!selectedCustomerId} style={selectedCustomerId ? { opacity: 0.7, cursor: 'not-allowed' } : {}} onChange={e => {
              setBuyerName(e.target.value);
            }} placeholder=" شرکت...  " />
              </div>

              <div className="invoice-field-group">
                <label className="invoice-field-label">کد اقتصادی خریدار</label>
                <input type="text" className="invoice-input-text" value={buyerEconomicCode} readOnly={!!selectedCustomerId} style={selectedCustomerId ? { opacity: 0.7, cursor: 'not-allowed' } : {}} onChange={e => setBuyerEconomicCode(toPersianDigits(e.target.value))} placeholder=" ...123 " />
              </div>
            </div>

            <div className="invoice-form-grid-2">
              <div className="invoice-field-group">
                <label className="invoice-field-label">شماره تماس خریدار</label>
                <input type="text" className="invoice-input-text" value={toPersianDigits(buyerMobile)} readOnly={!!selectedCustomerId} style={selectedCustomerId ? { opacity: 0.7, cursor: 'not-allowed' } : {}} onChange={e => setBuyerMobile(e.target.value)} placeholder=" ...0912 " />
              </div>

              <div className="invoice-field-group">
                <label className="invoice-field-label">نشانی خریدار</label>
                <input type="text" className="invoice-input-text" value={buyerAddress} readOnly={!!selectedCustomerId} style={selectedCustomerId ? { opacity: 0.7, cursor: 'not-allowed' } : {}} onChange={e => setBuyerAddress(e.target.value)} placeholder="استان، شهر، خیابان..." />
              </div>
            </div>
          </div>
          ...
          <div className="invoice-form-card">
            <div className="invoice-form-section-header">
              ...
              <button type="button" className="invoice-add-row-btn" onClick={addRow} disabled={rows.length >= MAX_ROWS}>
                ➕ افزودن سطر 
              </button>
            </div>
            ...
            <div className="invoice-form-items-list">
              {rows.map((row, idx) => {
            const qtyVal = row.quantity === "" || row.quantity === undefined ? 1 : parseNumber(row.quantity) || 1;
            const priceVal = row.unitPrice === "" || row.unitPrice === undefined ? 0 : parseNumber(row.unitPrice) || 0;
            return <div key={row.id} className="invoice-form-item-row">
                    <div className="item-row-top-bar">
                      <span className="item-row-badge">سطر {toPersianDigits(idx + 1)}</span>
                      {rows.length > 1 && <button type="button" className="item-row-delete-btn" onClick={() => removeRow(idx)} title="حذف این سطر">
                          🗑️ حذف 
                        </button>}
                    </div>
                    <div className="item-row-fields">
                      <div className="item-field-desc">
                        <label className="item-label-sm">شرح کالا / خدمات</label>
                        <input type="text" className="invoice-input-text" value={row.desc} onChange={e => updateRow(idx, "desc", e.target.value)} placeholder="محصول جدید" />
                      </div>
                      <div className="item-field-qty">
                        <label className="item-label-sm">تعداد</label>
                        <input type="text" className="invoice-input-text text-center" value={row.quantity === "" || row.quantity === undefined ? "" : toPersianDigits(row.quantity)} onChange={e => updateRow(idx, "quantity", e.target.value === "" ? "" : parseNumber(e.target.value))} placeholder="۱" />
                      </div>
                      <div className="item-field-price">
                        <label className="item-label-sm">قیمت واحد (ریال)</label>
                        <input type="text" className="invoice-input-text text-left" value={row.unitPrice === "" || row.unitPrice === 0 || row.unitPrice === undefined ? "" : formatNumber(row.unitPrice)} onChange={e => updateRow(idx, "unitPrice", e.target.value === "" ? "" : parseNumber(e.target.value))} placeholder="۰" />
                      </div>
            ...
            <div className="invoice-form-total-summary">
              ...
              <div className="summary-amount">
                {formatNumber(grandTotal)} <span className="currency">ریال</span>
              </div>
            </div>
          ...
          <div className="invoice-form-card">
            ...
            <textarea ref={noteTextareaRef} className="invoice-input-textarea" value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="توضیحات یا شرایط فاکتور را وارد کنید..." />
          </div>
          ...
          <div className="invoice-bottom-bar" style={{ opacity: 1, animation: 'none' }}>
            <div className="invoice-bottom-actions-card" style={{ position: 'relative' }}>
              ...
              <button type="button" className="action-btn" style={{ background: 'linear-gradient(180deg, #3282b8, #0f4c75)', width: 40, height: 40, borderRadius: '50%', color: 'white', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(15, 76, 117, 0.4)', padding: 0 }} onClick={() => setStep("preview")} title="پیش نمایش">
                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
              </button>
              <button type="button" className="action-btn save-ledger-btn" style={{ background: 'linear-gradient(180deg, #10b981, #059669)', width: 40, height: 40, borderRadius: '50%', color: 'white', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(16, 185, 129, 0.4)', padding: 0 }} onClick={handleSaveToDatabase} disabled={isSaving} title="ذخیره در حساب مشتری">
                {isSaving ? (
            ...
            >
              ...
              >
                <div style={{ padding: "500px" }}>
                  <div className="invoice-card" id="reportCard" ref={invoiceCardRef} dir="rtl">
              <div className="invoice-inner" id="invoice-content">
                  {/* Title Section */}
                <div className="header-title">
                  <div className="header-title-center">
                    <h1>{invoiceType}</h1>
                    <div className="sub-brand">{sellerName || "نام شرکت"}</div>
                  </div>

                  {(companyLogo || sellerEconomicCode) && <div className="header-logo-economic-left">
                      {companyLogo && <div className="invoice-header-logo-wrapper">
                          <img src={companyLogo} alt="لوگوی شرکت" className="invoice-header-logo" />
                        </div>}
                      {sellerEconomicCode && <div className="seller-economic-code-badge">
                ...
                <div className="invoice-meta">
                  <div className="meta-item">
                    ...
                    <span className="editable-date-text">{toPersianDigits(invoiceDate)}</span>
                  </div>
                  <div className="meta-item">
                    <span className="meta-label">شماره فاکتور :</span>
                    <span className="editable-invno-text" dir="ltr" style={{ display: "inline-block", textAlign: "right" }}>{toPersianDigits(invoiceNumber)}</span>
                  </div>
                </div>
                  ...
                  <div className="buyer-box">
                    <div className="buyer-grid">
                      <div className="buyer-main-info">
                        <h2>{buyerName}</h2>
                        {buyerAddress && <p className="buyer-info-text">
                            آدرس : {buyerAddress}
                          </p>}
                      </div>
                      <div className="buyer-extra-info">
                        {buyerEconomicCode && <p className="buyer-info-text">
                            کد اقتصادی : {toPersianDigits(buyerEconomicCode)}
                          </p>}
                        {buyerMobile && <p className="buyer-info-text">
                            شماره تماس : {toPersianDigits(buyerMobile)}
                          </p>}
                      </div>
                    </div>
                  </div>
                ...
                <div className="app-footer">
                  ...
                  <span className="footer-app-name">{sellerName || "نام شرکت"}</span>
                  {sellerAddress && <div className="footer-address">{sellerAddress}</div>}
                  {sellerPhone && <div className="footer-phone">{toPersianDigits(sellerPhone)}</div>}
                </div>
              </div>
            </div>
                </div>
...
{exportModal.show && (
              ...
              <button type="button" className="action-btn save-ledger-btn" style={{ background: 'linear-gradient(180deg, #10b981, #059669)', width: 40, height: 40, borderRadius: '50%', color: 'white', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(16, 185, 129, 0.4)', padding: 0 }} onClick={handleSaveToDatabase} disabled={isSaving} title="ثبت در حساب مشتری">
                {isSaving ? (
                    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="spinning-icon"><line x1="12" y1="2" x2="12" y2="6"></line><line x1="12" y1="18" x2="12" y2="22"></line><line x1="4.93" y1="4.93" x2="7.76" y2="7.76"></line><line x1="16.24" y1="16.24" x2="19.07" y2="19.07"></line><line x1="2" y1="12" x2="6" y2="12"></line><line x1="18" y1="12" x2="22" y2="12"></line><line x1="4.93" y1="19.07" x2="7.76" y2="16.24"></line><line x1="16.24" y1="4.93" x2="19.07" y2="7.76"></line></svg>
                ) : (
                    <svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
                )}
              </button>
              <button type="button" className="action-btn save-btn" style={{ background: 'linear-gradient(180deg, #3282b8, #0f4c75)', width: 40, height: 40, borderRadius: '50%', color: 'white', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(15, 76, 117, 0.4)', padding: 0 }} onClick={saveAsImage} disabled={isSaving} title="ذخیره به عنوان تصویر">
                <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
              </button>
      ...
      {/* تقویم شمسی اختصاصی */}
      <JalaliDatePickerModal isOpen={isDatePickerOpen} initialDate={invoiceDate} onSelectDate={formattedDate => setInvoiceDate(formattedDate)} onClose={() => setIsDatePickerOpen(false)} />
      ...
      {/* Custom Alert Modal */}
      {customAlert.show && (
        <div className="custom-alert-overlay" onClick={customAlert.onCancel} style={{
            position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', 
            background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 999999, 
            display: 'flex', alignItems: 'center', justifyContent: 'center', 
            animation: 'fadeIn 0.3s ease'
        }}>
          <div className="custom-alert-box" onClick={e => e.stopPropagation()} style={{
              background: '#0f4c75', border: '1px solid rgba(255,255,255,0.1)',
              borderRadius: '16px', padding: '25px', width: '90%', maxWidth: '350px',
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px',
              boxShadow: '0 10px 40px rgba(0,0,0,0.5)', animation: 'popIn 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)',
              color: 'white', textAlign: 'center'
          }}>
            <div className="custom-alert-icon" style={{
            ...
            <div className="custom-alert-message" style={{
                fontSize: '1.05rem', fontWeight: '600', lineHeight: '1.6'
            }}>
              {customAlert.message}
            </div>
            <div className="custom-alert-actions" style={{
                display: 'flex', gap: '10px', width: '100%', marginTop: '5px'
            }}>
              {customAlert.type === 'confirm' ? (
                <>
                  <button onClick={customAlert.onConfirm} style={{
                      flex: 1, padding: '12px', borderRadius: '10px', border: 'none',
                      background: 'linear-gradient(180deg, #10b981, #059669)', color: 'white',
                      fontWeight: '700', fontSize: '1rem', cursor: 'pointer',
                      boxShadow: '0 4px 10px rgba(16, 185, 129, 0.4)'
                  }}>بله</button>
                  <button onClick={customAlert.onCancel} style={{
                      flex: 1, padding: '12px', borderRadius: '10px', border: 'none',
                ...
                <button onClick={customAlert.onConfirm} style={{
                    flex: 1, padding: '12px', borderRadius: '10px', border: 'none',
                    background: 'linear-gradient(180deg, #3282b8, #0f4c75)', color: 'white',
                    fontWeight: '700', fontSize: '1rem', cursor: 'pointer'
                }}>تایید</button>
              )}
            </div>
        ...
        <div className="invoice-success-modal-overlay">
          <div className="invoice-success-modal">
            <div className="invoice-success-icon">
              <svg viewBox="0 0 24 24" width="60" height="60" fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
                <polyline points="22 4 12 14.01 9 11.01"></polyline>
              </svg>
            </div>
            <h3 className="invoice-success-title">{saveSuccessMessage}</h3>
            <button className="invoice-success-close-btn" onClick={handleAlertCloseAndNavigate}>تایید</button>
          </div>
        </div>
      )}
    </div>
