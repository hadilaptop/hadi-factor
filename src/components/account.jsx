...
function Account({
  ...
  const handleSave = async () => {
    ...
    const isDuplicate = existingCustomers.some(c => {
      ...
      return c.name && c.name.trim().toLowerCase() === trimmedName.toLowerCase();
    });
    if (isDuplicate) {
      setAlertModal({
        show: true,
  ...
  return (
    <div id="account-view" className="account-overlay">
      <div className="account-wrapper">
          ...
          <div className="crm-header-info">
            ...
            {customerToEdit && (
              <span className="account-subtitle">
                کد مشتری: {toPersianDigits(getCustomerCode(customerToEdit, existingCustomers))}
              </span>
            )}
          </div>
          <div className="crm-header-buttons">
            <button className="crm-back-btn" onClick={() => onNavigate("dashboard")} title="داشبورد">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
                <polyline points="9 22 9 12 15 12 15 22"></polyline>
              </svg>
            </button>
            <button id="closeAccountBtn" className="crm-back-btn" onClick={handleClose} title="بازگشت">
        ...
        <div id="account-content" className="account-content">
          <form className="account-form-card" onSubmit={e => { e.preventDefault(); handleSave(); }}>
            <div className="account-input-group">
              <label>نام مشتری / شرکت :</label>
              <input type="text" id="custNameInput" className="account-input" placeholder=" شرکت... " value={customerName} onChange={e => setCustomerName(e.target.value)} />
            </div>
            <div className="account-input-group">
              <label>آدرس :</label>
              <input type="text" id="custAddressInput" className="account-input" placeholder="استان، شهر، خیابان..." value={address} onChange={e => setAddress(e.target.value)} />
            </div>
            <div className="account-input-group">
              <label>شماره تماس :</label>
              <input type="text" id="custPhoneInput" className="account-input" dir="ltr" placeholder="0912..." value={phone} onChange={e => setPhone(e.target.value)} />
            </div>
            <div className="account-input-group">
              <label>کد اقتصادی :</label>
              <input type="text" id="custEconomicCodeInput" className="account-input" dir="ltr" placeholder="0" value={economicCode} onChange={e => setEconomicCode(e.target.value)} />
            </div>
            ...
            <div className="account-modal-actions">
              <button type="submit" id="saveCustomerBtn" className="account-btn-save" onMouseDown={e => e.preventDefault()}>
      ...
      {alertModal.show && (
        <div className={`custom-alert-overlay ${isClosingAlert ? "is-closing" : ""}`} onClick={closeAlertModal}>
          <div className="custom-alert-box" onClick={e => e.stopPropagation()}>
            <div className="custom-alert-icon-bg">
              <svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="#f59e0b" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <p className="account-custom-element">
              {alertModal.message}
            </p>
